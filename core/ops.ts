...
export function formatHarvestedCardsSection(
  ...
  if (diagnosticsEnabled) {
    ...
    if (overBudgetCards.length > 0) lines.push(isEn ? `> ⚠️ ${overBudgetCards.length} card(s) exceed per-card body budget (${maxCardChars} chars)` : `> ⚠️ ${overBudgetCards.length} 张卡片超出单卡正文预算（${maxCardChars} 字符）`);
