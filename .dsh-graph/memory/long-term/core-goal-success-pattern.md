...
## 复核逐条验证 + 决策点写死判据（2026-08-22 g-130 两轮打回）

- g-130 子代理两次声称「判据全过」但 validate 重复小节不变式（负责人决策点 3）未实现——
  复核必须逐条对照判据验证实现（不只信测试数），决策点要写死进判据防遗漏；
- 我自己复核时发现 validate 正则 `/## 目标描述/g` 匹配正文内引用字样 → 误报 g-130 自身
