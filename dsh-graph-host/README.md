...
>
> **依赖说明**：由 DSH 宿主提供的核心包（`@deepseek-ai/cordis` ^4.0.2、`@deepseek-ai/schemastery` ^3.18.2、`@deepseek-ai/dsh-settings` ^0.1.5-rc.2）声明为 `peerDependencies`，插件自有依赖（`yaml`）作为 `dependencies` 安装，避免产生重复的核心包实例。首次安装时出现的 peer 提示属于宿主 profile 配置（`autoInstallPeers: false`）的固有产物，由宿主环境统一注入提供。
>
...
>
> **Dependency Note**: Core packages provided by the DSH host (`@deepseek-ai/cordis` ^4.0.2, `@deepseek-ai/schemastery` ^3.18.2, `@deepseek-ai/dsh-settings` ^0.1.5-rc.2) are declared under `peerDependencies`. Plugin-specific dependencies (`yaml`) are retained in `dependencies`, preventing duplicate core package instances. Notice that peer dependency warnings during initial installation are a natural consequence of the host profile configuration (`autoInstallPeers: false`), as these packages are provided directly by the host runtime.
>
...
>
> **⚠️ Platform scope**: the verification above was done on **Linux (WSL2)** and does **not** cover native Windows. Two known independent problems on Windows: (1) `core/ops.ts` uses POSIX-only file-lock constants (directory-as-fd, `O_DIRECTORY`, `O_NOFOLLOW`); (2) host-provided core packages are declared in both `dependencies` and `peerDependencies`, tripping the DSH profile core-package isolation check. Tracked as board goals g-284 / g-285; do not deploy on native Windows until they are fixed and re-verified there (WSL2 is unaffected).
>
