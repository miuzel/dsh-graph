...
test("g-118/g-119：注入不影响 graph_* 工具注册（16 + bind + help + rename + archive/unarchive = 21）", () => {
  const ws = mkdtempSync(join(tmpdir(), "dsh-graph-g118-"));
  const root = resolveRoot({}, ws);
  init(root);
  writeFileSync(join(root, "project.yaml"), "supervisor:\n  session: session-super-1\n");
  ...
  apply(ctx, { root });
  assert.equal(registered.length, 21, "g-116 16 + g-119 graph_bind_collect_card + g-118 graph_help + g-141 graph_rename_goal + g-110 archive/unarchive = 21");
