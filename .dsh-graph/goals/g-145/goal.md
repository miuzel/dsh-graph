...
1. 通过工具/后端派发的收集子代理，其实际 spawn 提示明确包含**真实 workspace/仓库根**（绝不能把 `<workspace>/.dsh-graph` 数据根标成仓库根）、当前 goal id/标题、当前 card id/标题/kind、收集范围与精确 `graph_fill_card` 回填模板；子代理无需猜测任何 `.dsh-graph` 路径。
2. 从 GUI 点击“开始收集”触发的收集子代理，生成的实际提示词也必须包含同一组目标/卡片上下文与**真实 workspace/仓库根**；不得走缺字段、与工具路径不一致或将数据根误标为仓库根的 prompt 分支。
...
5. 新增覆盖工具派发与 GUI“开始收集”两条路径的提示上下文完整性、真实 workspace 根、正确回填、错 goal/card 回填拒绝（或可识别失败）的回归测试；`node --test core/tests/*.test.ts` 全量通过。
