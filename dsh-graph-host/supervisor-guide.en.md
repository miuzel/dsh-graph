...
- **Boundary (stated honestly)**: "strict must dispatch an independent review subagent" is only a decision plus a guide constraint at the plugin layer, not engine enforcement—the plugin layer has no reviewer dispatch entry point yet (g-248 not wired); `delivered` is still reachable only through the existing accept and human paths, and the fast track adds no route around the human gate.
