...
import { existsSync, lstatSync, fstatSync, openSync, closeSync, fsyncSync, fchmodSync, chmodSync, readFileSync, renameSync, unlinkSync, } from "node:fs";
...
export function withPlatformForTesting(platform, fn) {
 ...
 */
export function areSameStat(statA, statB) {
    if (isWindows())
        return true;
    if (!statA || !statB)
        return false;
 ...
 */
export function syncDirectorySafely(dirPath) {
    if (isWindows())
        return;
    try {
 ...
 */
export function applyModeSafely(fdOrPath, mode) {
    if (isWindows())
        return;
    try {
 ...
 * 获取文件当前同一性快照。
 */
export function takeFileIdentity(filePath, content) {
    const win = isWindows();
 ...
 */
export function verifyFileIdentity(filePath, expected, fd) {
