...
>
> **依赖说明**：宿主提供的核心包（`@deepseek-ai/cordis` ^4.0.2、`@deepseek-ai/schemastery` ^3.18.2）作为 `peerDependencies` 由 DSH 宿主环境提供，插件自有依赖（`yaml`）打包在 `dependencies` 中，避免产生重复的核心包实例与未满足的 peer 告警。
>
