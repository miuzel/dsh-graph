...
import { ensureWatcher, watcherSafe, generation, invalidate, inspectGenerations, closeWatchers } from "./cache-state.ts";
export { closeWatchers };
export interface CachedBoardEntry { etag:string; revision:string; payload:any; payloadJson:string; cachedAt:number; }
...
export function getCachedBoardPayload(root:string,opts?:{includeArchived?:boolean}, payloadFactory?: (root:string, opts:{includeArchived:boolean}) => any) {
  ...
  if (safe && old?.revision===stableRevision) return {...old,fromCache:true};
