...
export function withMemoryLock<T>(root: string, fn: () => T): T {
  ...
  for (let i = 0; i < 600; i++) {
    try {
      ...
      writeSync(fd, lease, undefined, "utf8"); fsyncSync(fd); break;
    } catch (e: any) {
  ...
  if (fd < 0) throw new Error("memory lock timeout");
...
export function readMemoryEvents(root: string): GraphEvent[] {
...
export function memoryDiagnostics(root: string): string[] {
  const file = join(root, "memory", "memory.jsonl"); if (!existsSync(file)) return [];
  const out: string[] = [];
  for (const [i, raw] of readFileSync(file, "utf8").split("\n").entries()) {
    const line = raw.trim(); if (!line) continue;
    try {
      const rec = JSON.parse(line);
      if (!rec || typeof rec !== "object" || Array.isArray(rec) || typeof rec.event !== "string" || typeof rec.actor !== "string") out.push(`memory.jsonl 第 ${i + 1} 行 schema 非法`);
      else if (!["memory.added", "memory.replaced", "memory.removed"].includes(rec.event)) out.push(`memory.jsonl 第 ${i + 1} 行事件类型非法`);
    } catch { out.push(`memory.jsonl 第 ${i + 1} 行不是合法 JSON`); }
  }
  return out;
}

export function replayMemory(events: GraphEvent[]): MemoryEntry[] {
  ...
  for (const ev of events) {
    if (ev.event === "memory.added") {
      ...
      if (kind === "user") Object.defineProperty(entry, "owner", { value: ev.actor, enumerable: false, writable: true });
      ...
      if (existing) {
        ...
        if (kind === "user" && owner) Object.defineProperty(updated, "owner", { value: owner, enumerable: false, writable: true });
