 ...
 */
export function normalizeDescriptionHeadings(text) {
...
export function listGoalFiles(root, opts) {
    const out = [];
    ...
    if (existsSync(backlog)) {
        for (const f of readdirSync(backlog)) {
            if (f === "archived") {
                if (includeArchived) {
                    ...
                    for (const af of readdirSync(backlogArchived)) {
                        if (af.endsWith(".md"))
                            out.push(join(backlogArchived, af));
            ...
            if (f.endsWith(".md")) {
                ...
                if (!includeArchived && isArchivedFile(fp))
                    continue;
                out.push(fp);
                continue;
            }
...
export const SUBAGENT_MODES = ["standard", "minimal"];
...
const AUTOMATION_VALUES = ["human", "ai"];
const PROMPT_OVERRIDE_KEYS = ["subagent"];
/** 行缩进长度（空格/tab 计长）。 */
function lineIndent(l) {
    return /^[ \t]*/.exec(l)[0].length;
}
/** 在 lines[start..end) 内找「恰好为 indent 缩进的 key[:]」行，返回行号或 -1。 */
function findKeyLine(lines, key, indent, start, end) {
 ...
 *  字段行）不干扰块边界。 */
function blockChildrenEnd(lines, blockIdx, blockIndent) {
    for (let i = blockIdx + 1; i < lines.length; i++) {
        const l = lines[i];
        if (l.trim() === "")
            continue;
        if (/^[ \t]*#/.test(l))
            continue; // comment-only line: 不参与块边界
        if (lineIndent(l) <= blockIndent)
            return i;
    }
    return lines.length;
}
...
function readScalarByPath(lines, path) {
...
export function readPromptOverride(root, key) {
    const file = join(root, "project.yaml");
    if (!existsSync(file))
...
function readPromptOverrideConfig(lines, key) {
    const poIdx = findKeyLine(lines, "prompt_overrides", 0, 0, lines.length);
    if (poIdx < 0)
        return { value: null }; // 未配置 → default
    ...
    if (kIdx < 0)
        return { value: null }; // 未配置该键 → default
    const raw = lines[kIdx].slice(lineIndent(lines[kIdx]) + key.length + 1);
    const trimmed = raw.trim();
    if (trimmed === "" || trimmed === "null" || trimmed === "~")
        return { value: { state: "disable", value: null } };
    if (trimmed === "default")
        return { value: { state: "default", value: null } };
    if (trimmed === '""' || trimmed === "''")
        return { value: { state: "disable", value: null } };
...
export function readProjectConfig(root) {
    ...
    const lines = readFileSync(file, "utf8").split("\n");
    const scal = (path) => readScalarByPath(lines, path);
    const auto = {};
...
function validateConfigPatch(patch) {
    if (patch === null || typeof patch !== "object")
        throw new GraphError("配置必须是对象");
    const needObj = (v, name) => { if (v !== undefined && v !== null && typeof v !== "object")
        throw new GraphError(`${name} 必须是对象`); };
    const needStr = (v, name, { nullable = false, nonEmpty = false } = {}) => {
        if (v === undefined || v === null) {
            if (!nullable)
                throw new GraphError(`${name} 不能为空`);
            return;
        }
        if (typeof v !== "string")
            throw new GraphError(`${name} 必须是字符串`);
        if (nonEmpty && v.trim() === "")
            throw new GraphError(`${name} 不能为空`);
    };
    ...
    if ("defaults" in patch) {
        ...
        if ("lanes" in pk) {
            ...
            if (lanes !== null && lanes !== undefined && (!Number.isInteger(lanes) || lanes < 1))
                throw new GraphError("defaults.pk.lanes 必须是 >=1 的整数");
        }
    ...
    if ("supervisor" in patch) {
        ...
        for (const k of AUTOMATION_KEYS) {
            ...
            if (v === null || v === undefined)
                continue;
            if (typeof v !== "string" || !AUTOMATION_VALUES.includes(v)) {
    ...
    if ("prompt_overrides" in patch) {
        ...
        for (const key of PROMPT_OVERRIDE_KEYS) {
            ...
            const o = v ?? {};
            if (o.state !== undefined && !["default", "override", "disable"].includes(o.state))
                throw new GraphError(`prompt_overrides.${key}.state 只允许 default/override/disable`);
...
export function writeProjectConfig(root, patch, actor) {
    ...
    const setScalar = (path, value, encode) => {
        const enc = encode ?? ((v) => (v === null || v === undefined || v === "" ? "" : String(v)));
        setScalarAtPath(lines, path, value, enc);
    };
    if (patch.executor) {
        if ("provider" in patch.executor)
    ...
    if (patch.defaults) {
        const d = patch.defaults ?? {};
        if (d.review) {
            if ("reviewer" in d.review)
                setScalar(["defaults", "review", "reviewer"], d.review.reviewer ?? "");
            if ("prompt" in d.review)
                setScalar(["defaults", "review", "prompt"], d.review.prompt ?? "");
        }
        if (d.pk) {
            if ("lanes" in d.pk)
                setScalar(["defaults", "pk", "lanes"], String(d.pk.lanes ?? 1));
            if ("sandbox" in d.pk)
                setScalar(["defaults", "pk", "sandbox"], d.pk.sandbox ?? "");
        }
    }
    if (patch.supervisor && patch.supervisor.automation) {
        for (const k of AUTOMATION_KEYS) {
            if (k in patch.supervisor.automation)
                setScalar(["supervisor", "automation", k], patch.supervisor.automation[k] ?? "");
        }
    }
    if (patch.prompt_overrides) {
        for (const key of PROMPT_OVERRIDE_KEYS) {
            ...
            if (state === "disable")
                ...
                encoded = JSON.stringify(o.value ?? "");
...
function setScalarAtPath(lines, path, value, encode) {
    const ensureBlock = (parentIdx, parentEnd, childIndent, childKey) => {
...
function buildMissingChain(lines, path, leafValue) {
    // 去掉文末空行
 ...
 */
export function updateCriteria(root, id, opts) {
    // 规范化：trim + 丢弃空行
    ...
    if (base !== null) {
        const same = current.length === base.length &&
            current.every((c, i) => c === base[i]);
        if (!same) {
            if (opts.force === true)
                conflicted = true;
            else {
                throw new GraphConflictError("质量判据已被其他编辑修改（并发冲突）——请刷新后重试；或确认以本地内容覆盖");
            }
        }
    }
...
export function readGoalDirective(root, goalId) {
...
export function setGoalDirective(root, goalId, directive, actor) {
    if (typeof directive !== "string")
        throw new GraphError("directive 必须是 string 类型");
    const file = findGoalFile(root, goalId);
    const doc = loadGoal(file);
 ...
 *  无评论或小节不存在返回空数组。 */
export function readGoalComments(root, goalId) {
    const file = findGoalFile(root, goalId);
    const doc = loadGoal(file);
...
export function appendGoalComment(root, goalId, text, actor) {
    if (typeof text !== "string" || !text.trim())
        throw new GraphError("评论内容不能为空");
    const file = findGoalFile(root, goalId);
    const doc = loadGoal(file);
    const ts = nowIso();
...
export function formatGoalDirectiveSection(root, goalId) {
    ...
    if (existsSync(sdir)) {
        for (const f of readdirSync(sdir).sort()) {
            ...
            try {
                ...
                problems.push(`共享卡 ${cardId}: 解析失败：${e.message}`);
            }
        }
    }
...
export function assertSafeId(id, label) {
    const s = String(id ?? "");
    if (s === "")
        throw new GraphError(`${label} 不能为空`);
    if (/\0/.test(s))
        throw new GraphError(`${label} 含非法 NUL 字节`);
    if (isAbsolute(s))
        throw new GraphError(`${label} 不能是绝对路径`);
    if (s.includes(".."))
        throw new GraphError(`${label} 含非法路径片段（..）`);
    if (s.includes("/") || s.includes("\\"))
        throw new GraphError(`${label} 含非法路径分隔符`);
    return s;
}
/** 项目的独立共享卡池（与 versions/goals/backlog 平级；g-183）。 */
export function sharedCardsDir(root) {
    return join(root, "shared-cards");
}
const SHARED_CARD_PREFIX = "shared-";
/** 判断卡片 id 是否位于共享命名空间（仅用于快速判定；权威以 resolveCard 位置为准）。 */
function isSharedCardId(id) {
    return id.startsWith(SHARED_CARD_PREFIX);
}
/** 单张卡的摘要字段（供 board/详情/列表共用），scope 默认 "goal"。 */
function cardSummaryFields(meta, cardFilePath, scope) {
    return {
        id: meta.id,
        title: meta.title,
...
export function addCard(root, goalId, opts) {
    ...
    if (scope === "shared") {
        ...
        try {
            ...
            try {
                rmSync(join(sharedCardsDir(root), `${sharedId}.md`), { force: true });
            }
            catch { /* 忽略 */ }
            throw e;
        }
...
export function goalReferencesCard(root, goalId, cardId) {
...
export function resolveCard(root, goalId, cardId) {
    const goalIdSafe = assertSafeId(goalId, "goal id");
    ...
    if (basename(goalFile) === "goal.md") {
        ...
        if (existsSync(ownFile))
            return { file: ownFile, doc: loadGoal(ownFile), scope: "goal" };
    }
    ...
    if (existsSync(sharedFile)) {
        ...
        if (!goalReferencesCard(root, goalIdSafe, cardIdSafe)) {
            throw new GraphError(`共享卡 ${cardIdSafe} 未被目标 ${goalIdSafe} 引用，无法访问——请先挂载（addSharedCardRef）`);
        }
        return { file: sharedFile, doc: loadGoal(sharedFile), scope: "shared" };
    }
    throw new GraphError(`卡片不存在：${cardIdSafe}（目标 ${goalIdSafe}）`);
}
export function loadCard(root, goalId, cardId) {
...
export function addSharedCardRef(root, goalId, sharedId, actor) {
    const goalIdSafe = assertSafeId(goalId, "goal id");
...
export function referenceCount(root, sharedId) {
    const sharedIdSafe = assertSafeId(sharedId, "共享卡 id");
    let count = 0;
...
export function sharedCards(root) {
    const dir = sharedCardsDir(root);
    if (!existsSync(dir))
        return [];
    const out = [];
...
export function convertOwnedToShared(root, goalId, cardId, opts) {
    ...
    const newMeta = { ...doc.meta, id: newId, scope: "shared" };
    delete newMeta.goal;
    try {
        ...
        appendEvent(root, {
            ...
            goal: goalIdSafe,
            details: { card: cardId, from: "goal", to: newId, error: String(e.message), rollback: "ok" },
        });
        throw e;
    }
    ...
    try {
        ...
        if (idx < 0)
            throw new GraphError(`目标 ${goalIdSafe} 的 context_cards 中未找到引用 ${cardId}`);
        refs[idx] = newId;
...
export function convertSharedToOwned(root, goalId, cardId, opts) {
    const { file, doc, scope } = resolveCard(root, goalId, cardId);
    if (scope !== "shared") {
    ...
    if (refs !== 1) {
        throw new GraphError(`共享卡 ${cardId} 被 ${refs} 个 goal 引用，只有引用计数恰为 1 时才能转回 goal 自有卡——请先解除其余引用`);
    }
    ...
    const newMeta = { ...doc.meta, id: newId, scope: "goal", goal: goalIdSafe };
    try {
        saveGoal(newFile, { meta: newMeta, body: doc.body });
    }
    catch (e) {
        ...
        appendEvent(root, {
            ...
            event: "card.conversion_failed",
            goal: goalIdSafe,
            details: { card: cardId, from: "shared", to: newId, error: String(e.message), rollback: "ok" },
        });
        throw e;
    }
...
export function removeSharedCardRef(root, goalId, sharedId, actor) {
    const goalIdSafe = assertSafeId(goalId, "goal id");
    const sharedIdSafe = assertSafeId(sharedId, "共享卡 id");
    ...
    if (existsSync(sharedFile)) {
        ...
        if (sdoc.meta.status === "collecting") {
            throw new GraphError(`共享卡 ${sharedIdSafe} 正在收集中，不能解除引用——请先停止/完成收集子代理（否则绑定收集者回填会被成员守卫拒绝）`);
        }
    }
    const goalFile = findGoalFile(root, goalIdSafe);
    const goalDoc = loadGoal(goalFile);
...
export function deleteSharedCard(root, sharedId, opts) {
    const sharedIdSafe = assertSafeId(sharedId, "共享卡 id");
    ...
    const doc = loadGoal(file);
    if (doc.meta.status === "collecting") {
        throw new GraphError(`共享卡 ${sharedIdSafe} 正在收集子代理中，不能删除——请先停止子代理或等其完成`);
    }
    const refs = referenceCount(root, sharedIdSafe);
    if (refs > 0) {
        throw new GraphError(`共享卡 ${sharedIdSafe} 被 ${refs} 个 goal 引用，不能删除——请先在共享管理面板解除引用`);
    }
    // 事件先行（R-02）
    appendEvent(root, {
        actor: opts.actor,
...
function ensureAttachmentsRoot(root, createDirs = true) {
    ...
    if (!tryLstat(dir)) {
        if (!createDirs)
            return null; // 只读操作：根不存在即视为无附件，不创建目录
        mkdirSync(dir, { recursive: true });
    }
...
function attachmentsCanonicalPath(root) {
    const dir = attachmentsDir(root);
    if (tryLstat(dir)?.isSymbolicLink()) {
        throw new GraphError(`attachments 根不允许是 symlink：${dir}`);
    }
...
export function readAttachment(root, name) {
    const r = resolveAttachmentPath(root, name);
    if (!r)
        throw new GraphError(`附件不存在：${name}`);
...
export function storeAttachment(root, opts) {
    ...
    if (!r)
        throw new GraphError("附件存储失败：attachments 根不可用"); // createDirs=true 下根缺失会被创建，不应为 null
...
export function listAttachments(root) {
    ...
    if (rootSt.isSymbolicLink())
        throw new GraphError(`attachments 根不允许是 symlink：${attachmentsDir(root)}`);
...
export function attachmentInfo(root, name) {
...
export function attachmentReferenceCount(root, name) {
    ...
    for (const gfile of listGoalFiles(root, { includeArchived: true })) {
        try {
            bodies.push(loadGoal(gfile).body);
        }
        catch { /* 跳过 */ }
            ...
            if (existsSync(cdir)) {
                for (const f of readdirSync(cdir)) {
                    if (!f.endsWith(".md"))
                        continue;
                    try {
                        bodies.push(loadGoal(join(cdir, f)).body);
                    }
                    catch { /* 跳过 */ }
                }
            }
    ...
    if (existsSync(sdir)) {
        for (const f of readdirSync(sdir)) {
            if (!f.endsWith(".md"))
                continue;
            try {
                bodies.push(loadGoal(join(sdir, f)).body);
            }
            catch { /* 跳过 */ }
        }
    }
    ...
    for (const body of bodies)
        if (parseAttachmentRefs(body).includes(safeName))
            count++;
    return count;
}
            ...
            if (existsSync(cdir)) {
                for (const f of readdirSync(cdir)) {
                    if (!f.endsWith(".md"))
                        continue;
                    try {
                        checkBody(`${label}/卡片 ${f}`, loadGoal(join(cdir, f)).body);
                    }
                    catch { /* 跳过 */ }
                }
            }
    ...
    if (existsSync(sdir)) {
        for (const f of readdirSync(sdir)) {
            if (!f.endsWith(".md"))
                continue;
            try {
                checkBody(`共享卡 ${f}`, loadGoal(join(sdir, f)).body);
            }
            catch { /* 跳过 */ }
        }
    }
...
export function fillCard(root, goalId, cardId, opts) {
    ...
    if (doc.meta.status === "collecting" && doc.meta.child_id) {
        ...
        if (!isBoundChild && !isHuman) {
            ...
            if (scope === "shared") {
                throw new GraphError(`共享卡 ${cardId} 正在由 ${doc.meta.child_id} 收集，只有绑定的收集者（或 human override）可写入——请等待收集完成`);
            }
        }
    }
...
export function deleteCard(root, goalId, cardId, opts) {
    const { file, doc, scope } = loadCard(root, goalId, cardId);
    ...
    if (scope === "shared") {
        throw new GraphError(`共享卡 ${cardId} 被 goal 引用，不能删除——请在共享管理面板先解除引用（零引用后再显式删除）`);
    }
    // 事件先行（R-02）
    appendEvent(root, {
        actor: opts.actor,
...
export function bindCardChild(root, goalId, cardId, opts) {
    ...
    if (scope === "shared" && doc.meta.status === "collecting" && doc.meta.child_id && doc.meta.child_id !== opts.childId) {
        throw new GraphError(`共享卡 ${cardId} 正在由 ${doc.meta.child_id} 收集，不能并行收集——只允许一个权威收集者（如需重收先停止该子代理）`);
    }
    ...
    if (provider)
        doc.meta.provider = provider;
    else if (doc.meta.provider)
        delete doc.meta.provider;
    if (model)
        doc.meta.model = model;
    else if (doc.meta.model)
        delete doc.meta.model;
    saveGoal(file, doc);
    const details = { card: cardId, child_id: opts.childId };
    if (provider)
...
export function formatHarvestedCardsSection(root, goalId, opts, preHarvestedCards, language = "zh") {
    ...
    if (diagnosticsEnabled) {
        ...
        if (overTotal)
            lines.push(isEn ? `> ⚠️ Total output exceeds budget (${totalChars} > ${maxTotalChars})` : `> ⚠️ 总输出超出预算（${totalChars} > ${maxTotalChars}）`);
        if (overBudgetCards.length > 0)
            lines.push(isEn ? `> ⚠️ ${overBudgetCards.length} card(s) exceed per-card budget (${maxCardChars} chars)` : `> ⚠️ ${overBudgetCards.length} 张卡片超出单卡预算（${maxCardChars} 字符）`);
        if (overCount)
...
function validateConfirmedBy(root, confirmed_by) {
    ...
    if (confirmed_by.startsWith("supervisor:")) {
        ...
        if (configuredSession && sessionId === configuredSession)
            return;
        // 未配置 supervisor.session 时，允许 supervisor:* 前缀（首次 claim 前的引导阶段）
        if (!configuredSession)
            return;
        throw new GraphError(`确认身份 ${confirmed_by} 不匹配已配置的 supervisor.session（${configuredSession}）——只有已 claim 的主管会话或负责人可确认 handoff`);
    }
    ...
    if (confirmed_by.startsWith("agent:"))
        return;
    // 其他未知前缀仍拒绝
    throw new GraphError(`确认身份 ${confirmed_by} 前缀未知——仅支持 human:*、supervisor:* 或 agent:*`);
}
/** 主管/负责人登记 attempt handoff（g-150，单文件简化）。
 *  每个 goal 仅一个 handoff.md；新登记覆盖旧内容（旧历史由事件流保留）。
 *  事件先行：确认事件在 handoff 文件写入之前追加。 */
export function recordAttemptHandoff(root, goalId, opts) {
    // 校验确认身份可信
    ...
    if (!isNew) {
        try {
            ...
            const oldRev = oldDoc.meta.revision;
            if (typeof oldRev === "number" && oldRev >= 1)
                revision = oldRev + 1;
        }
        catch { /* 坏文件从 1 开始 */ }
    }
...
export function harvestReviewedAttemptHandoffs(root, goalId) {
    const goalFile = findGoalFile(root, goalId);
        ...
        try {
            ...
            if (raw.status === "confirmed" && raw.id && raw.confirmed_by && raw.confirmed_at) {
                const h = {
                    ...
                    goal: goalId,
                    status: "confirmed",
                    source_attempts: Array.isArray(raw.source_attempts) ? raw.source_attempts : [],
                ...
                if (h.failures && h.constraints && h.baseline && h.verification)
                    return [h];
            }
        }
        catch { /* 坏文件跳过 */ }
    ...
    for (const f of files) {
        try {
            ...
            if (raw.status !== "confirmed")
                continue;
            if (raw.superseded_by != null)
                continue;
            if (!raw.id || !raw.confirmed_by || !raw.confirmed_at)
                continue;
            if (!Array.isArray(raw.source_attempts) || !raw.failures || !raw.constraints || !raw.baseline || !raw.verification)
                continue;
            const supersedes = Array.isArray(raw.supersedes)
                ? raw.supersedes.filter((x) => typeof x === "string")
                : [];
            // 用 superseded 事件和 supersedes 链排除被淘汰的
            const supersededEvents = readEvents(root).filter((e) => e.event === "attempt.handoff.superseded" && e.goal === goalId);
            const supersededIds = new Set(supersededEvents.map((e) => e.details?.old_handoff).filter(Boolean));
            if (supersededIds.has(raw.id))
                continue;
            ...
            const isSupersededByOther = files.some((other) => {
                ...
                try {
                    ...
                    const otherMeta = otherDoc.meta;
                    if (otherMeta.status !== "confirmed")
                        return false;
                    const otherSupersedes = Array.isArray(otherMeta.supersedes) ? otherMeta.supersedes : [];
                    return otherSupersedes.includes(raw.id);
                }
                catch {
                    return false;
                }
            });
            if (isSupersededByOther)
                continue;
            all.push({
                id: raw.id,
                goal: goalId,
                status: "confirmed",
                source_attempts: raw.source_attempts,
...
export function formatCollectPrompt(root, goalId, cardId, userPrompt, language = "zh") {
    ...
    // 如果有用户提供的附加要求，追加在末尾
    if (userPrompt && userPrompt.trim()) {
        sections.push(``, `**用户附加要求**：`, userPrompt.trim());
    }
    return sections.join("\n");
}
/** 获取卡片元数据（供 GUI 收集 prompt 使用） */
export function getCardMeta(root, goalId, cardId) {
    const goalFile = findGoalFile(root, goalId);
...
function attemptWorktreeEvidence(root, goalId, attemptId) {
...
export function bindAttemptChild(root, goalId, attemptId, childId, actor, parentSessionId, provider, model, modelRoute, mode, modeSource) {
    ...
    if (provider && provider.trim())
        doc.meta.provider = provider.trim();
    if (model && model.trim())
    ...
    if (doc.meta.provider)
        details.provider = doc.meta.provider;
    if (doc.meta.model)
 ...
 *  无绑定返回 null。 */
export function readGoalBinding(root, goalId) {
    const goalFile = findGoalFile(root, goalId);
    if (basename(goalFile) !== "goal.md")
        return null; // backlog 平铺无 attempt
    ...
    for (const a of atts) {
        ...
        try {
            ...
            if (doc.meta.id !== a || doc.meta.goal !== goalId)
                continue; // 归属校验
            if (doc.meta.detached === true)
                continue; // 已解绑不算有效绑定
            if (doc.meta.executor === "agent:collect")
                continue; // 收集子代理不占 goal 执行绑定
...
export function authorizeUnbind(root, actor, createdBy, childId) {
    ...
    // 创建者 = owner
    if (createdBy && (a === createdBy || a === "agent:" + createdBy))
        return;
    ...
    if (a.startsWith("supervisor:")) {
        ...
        if (configured && sessionId === configured)
            return;
...
export function unbindGoalChild(root, goalId, opts) {
    ...
    const result = withTx({ root, actor, goal: goalId }, { lockName: "unbind-" + goalId }, () => {
        ...
        // 无绑定：幂等 no-op（不重复记事件）
        if (!binding) {
            return { value: { detached: false, already: true }, events: [] };
        }
...
export function moveGoal(root, id, opts) {
    ...
    const prevStatus = doc.meta.status;
    if (opts.to === "backlog") {
        if (srcDir) {
...
export function archiveGoal(root, id, opts) {
    const file = findGoalFile(root, id);
    const doc = loadGoal(file);
    ...
    if (existsSync(targetFile))
        throw new GraphError(`归档位置已存在：${targetFile}`);
    // 标记已归档
...
export function unarchiveGoal(root, id, opts) {
    const file = findGoalFile(root, id);
    const doc = loadGoal(file);
    ...
    if (existsSync(targetFile))
        throw new GraphError(`恢复位置已存在：${targetFile}`);
    // 清除归档标记
...
function isBacklogFile(file, root) {
    const rel = file.slice(root.length + 1);
...
export function postponeGoal(root, id, opts) {
    const file = findGoalFile(root, id);
    const doc = loadGoal(file);
    const rel = file.slice(root.length + 1);
    const srcDir = basename(file) === "goal.md" ? dirname(file) : null;
...
export function deleteGoal(root, id, opts) {
    const file = findGoalFile(root, id);
    const doc = loadGoal(file);
    // 前置校验 1：仅已归档目标可删除
...
export function goalCards(root, goalId) {
    ...
    for (const ref of Array.isArray(goalDoc.meta.context_cards) ? goalDoc.meta.context_cards : []) {
        ...
        const sharedFile = join(sharedCardsDir(root), `${id}.md`);
        if (!existsSync(sharedFile))
            continue; // 悬空引用（validate 管）；自有卡非 context_cards 已在上方扫出
        try {
...
export function renameGoal(root, id, opts) {
    const newTitle = opts.title.trim();
    if (!newTitle)
        throw new GraphError("标题不能为空");
    const file = findGoalFile(root, id);
    const doc = loadGoal(file);
    ...
    for (let i = 0; i < 200; i++) {
        ...
        try {
            ...
            if (!existing.isDirectory())
                throw new GraphError("标签锁路径不是目录，拒绝越界操作");
            if (Date.now() - existing.mtimeMs > 30_000) {
                const ownerPath = join(lock, "owner");
                const ownerStat = lstatSync(ownerPath);
                if (!ownerStat.isFile())
                    throw new GraphError("标签锁 owner 不是普通文件，拒绝回收");
                const owner = readFileSync(ownerPath, "utf8");
                if (!validToken(owner))
                    throw new GraphError("标签锁 owner 无效，拒绝回收");
            ...
            try {
                ...
                if (mkdirError instanceof GraphError && /不是目录|owner 无效|owner 不是/.test(mkdirError.message))
                    throw mkdirError;
            }
            const until = Date.now() + 5;
            while (Date.now() < until) { /* backoff */ }
...
export function setGoalTags(root, id, opts) {
    if (opts.force !== undefined && typeof opts.force !== "boolean")
        throw new GraphError("force 必须是布尔值");
...
export function readPromptOverrideValue(projectYamlText, key) {
