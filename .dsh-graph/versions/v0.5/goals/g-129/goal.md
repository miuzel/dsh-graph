...
1. 1. host 新增 /api/dsh-graph/create-goal 端点：POST title/version/scope，调 core createGoal，事件先行 goal.created
2. 2. GUI 看板主界面「新建目标」入口 + 弹窗（填标题、可选 version/scope）
3. 3. core goalDetail 暴露 goal.md 路径（相对 workspace 或绝对）
4. 4. GUI 目标详情/卡片抽屉提供「打开 goal.md」链接：可复制路径 + 打开动作（用户可用编辑器自由编辑）
5. 5. 全量测试与冻结脚本 PASS，graph_validate 无问题
