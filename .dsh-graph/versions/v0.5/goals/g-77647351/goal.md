...
3. 跨列拖动触发状态迁移：列 key↔状态枚举映射（STAGES）；blocked 只能回 blocked_from（需 reason）、delivered 终态需确认、in_progress 门槛服务端报错兜底、planning→collect 二义默认 collecting
4. 排序持久化：.dsh-graph/order.json（v-slug|standalone|backlog × 列 → goalId[]）+ GET/POST order 端点，workspace reconciledSessionOrder 对账模式
