...
export class TxError extends GraphError {
    phase;
    recoverable;
    constructor(message, phase, recoverable = false) {
        super(message);
        this.phase = phase;
...
export class TxCasError extends TxError {
    constructor(message, phase = "read") {
...
export function withTx(ctx, opts, fn) {
    ...
    try {
        ...
        return { ok: false, phase, error: String(e?.message ?? e), recoverable: false };
    }
}
...
export function casWrite(file, content, expect) {
...
export function readModifyWrite(file, opts) {
