...
import { init, createGoal, setCriteria, transition, validate, rebuild, addCard, fillCard, reviewCard, startAttempt, reportStatus, moveGoal, amendGoal, deleteGoal, archiveGoal, unarchiveGoal, deleteCard, postponeGoal, unbindGoalChild, abandonAttempt, } from "./ops.js";
...
function main() {
    ...
    switch (args.command) {
        ...
        default:
            throw new GraphError("用法：node core/main.ts [--root DIR] <init|create-goal|set-criteria|transition|add-card|fill-card|review-card|delete-card|start-attempt|report-status|move-goal|amend-goal|archive-goal|unarchive-goal|delete-goal|postpone-goal|unbind-goal-child|abandon-attempt|validate|rebuild> [flags]");
    }
}
try {
    main();
