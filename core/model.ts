 ...
 */
export function computeClosedFenceMask(lines: string[]): boolean[] {
...
/**
 * 边界定位结果：包含起始小节标题行索引 start、下一小节标题行索引 end（或 EOF）及按行拆分后的 lines 数组。
 */
 ...
 * 若小节不存在返回 null。
 */
export function findSectionBounds(body: string, name: string): SectionBounds | null {
...
export function replaceSection(
  ...
  const bounds = findSectionBounds(body, name);
  const head = `## ${name}`;
  if (!bounds) throw new Error(`小节不存在：${head}`);
